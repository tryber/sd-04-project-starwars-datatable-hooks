...
export const getColumns = (onColumnChange, numericValues, column) => {
  ...
  return (
    <select onChange={(event) => onColumnChange(event)} data-testid="column-filter" value={column}>
      {select.map((option) => (
...
export const getColumnsToSort = (onColumnChange, column) => {
  ...
  return (
    <select data-testid="column-sort" onChange={(event) => onColumnChange(event)} value={column}>
      {select.map((item) => (
